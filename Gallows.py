...
#music
list_music=["music/background.mp3","music/background_2.mp3",
            "music/background_3.mp3","music/background_4.mp3",
            "music/background_5.mp3","music/background_6.mp3",
            "music/background_7.mp3"]
NUM_MUSIC_NOW=0
...
song_next=Button(screen,25,25)
song_last=Button(screen,25,25)
next_song = Element(screen,"images/next.png")
last_song = Element(screen,"images/last.png")
...
def swap_song(go=None):
    global NUM_MUSIC_NOW
    if NUM_MUSIC_NOW==len(list_music)-1:
        NUM_MUSIC_NOW=0
    if go==None:
        pygame.mixer.music.load(list_music[NUM_MUSIC_NOW])
...
def play_music():
    song_next.draw_circle(570,570,25,swap_song,go=True)
    song_last.draw_circle(30,570,25,swap_song,go=False)

    next_song.output(-270,5)
    last_song.output(270,5)
...
def event_loop(player: Game,points):
    while True:
...
def hint_menu(player: Game,points):
    def rand_let(player: Game,points):
        if (points-3)<3:
            return True
        else:
            ...
            return True
    def first_let(player: Game,points):
        if (points-5)<5:
            return True
        else:
            with open("points.txt","w") as f:
            ...
            return True
    def last_let(player: Game,points):
        if (points-4)<4:
            return True
        else:
            with open("points.txt","w") as f:
            ...
            return True
    def back_screen(player: Game,points):
        player.word
        return True
...
def easy():
    ...
    while True:
        ...
        if player.tries>=1:
            stick_1.output(80,120)
            if player.tries>=2:
                stick_2.output(80,120)
                if player.tries>=3:
                    rope.output(80,120)
                    if player.tries>=4:
                        head.output(80,120)
                        if player.tries>=5:
                            ...
                            if player.tries>=6:
                                ...
                                if player.tries>=7:
                                    arm_2.output(80,120)
                                    if player.tries>=8:
                                        foot_1.output(80,120)
                                        if player.tries>=9:
                                            foot_2.output(80,120)
                                            eyes.output(80,120)
        if player.check_tries(): 
            lose(screen,player,easy)
        if player.finish_game():
            win(screen,scores,20,points,easy)
        text=ARIAL_45.render(player.enter_letter(event_loop(player,points)), True,COLOR_FONT)
